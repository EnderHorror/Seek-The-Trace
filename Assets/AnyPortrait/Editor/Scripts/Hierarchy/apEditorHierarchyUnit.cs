			...
			switch (_unitType)
			{
				...
				case UNIT_TYPE.ToggleButton_Visible:
					if (!_isSelected)
					{
						...
						if(_isCur_VisiblePostfix)
						{
							isBtnClick = GUILayout.Button(_guiContent_Text_Short.Content, _isAvailable ? _guiStyle_None : _guiStyle_NoAvailable, apGUILOFactory.I.Width(width - (_cursorX + 30)), _layoutOption_H_Height);
						}
						else
						{
							isBtnClick = GUILayout.Button(_guiContent_Text.Content, _isAvailable ? _guiStyle_None : _guiStyle_NoAvailable, _layoutOption_H_Height);
						}
						if (isBtnClick)
						{
							if (_funcClick != null)
