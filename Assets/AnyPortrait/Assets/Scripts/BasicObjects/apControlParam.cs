...
namespace AnyPortrait
{
	...
	public class apControlParam
	{
		...
		//이전 방식 (레이어 기능이 없다.)
		//public void SetCalculated_Int(	//int iValue,//이전
		//								float iValue_Float,//변경 20.4.15 : 보간 버그를 해결하기 위해 최종 결과를 제외하고는 모두 float형으로 다룬다. 
		//								float weight, apAnimPlayUnit.BLEND_METHOD blendMethod)
		//{
		...
		//	{
		//		//_int_CalculatedLayer = (int)(_int_CalculatedLayer * (1.0f - weight) + iValue * (weight) + 0.5f);//이전 (int형으로 변환)
		//		_int_CalculatedLayer = (_int_CalculatedLayer * (1.0f - weight)) + (iValue_Float * weight);//변경 (float형을 유지) 20.4.15
		//	}
		//	else
		//	{
		//		//_int_CalculatedLayer += (int)(iValue * weight + 0.5f);//이전 (int형으로 변환)
		//		_int_CalculatedLayer += iValue_Float * weight;//변경 (float형으로 계산) 20.4.15
		//	}
		//	_isUpdated = true;
		//}
		...
		public void AddCalculated_Request(apAnimControlParamResult calculatedRequest, int layer)
		{
			...
			//{
			//	case TYPE.Int:
			//		strValue = calculatedRequest._value_Int.ToString();
			//		break;

			//	case TYPE.Float:
			//		strValue = calculatedRequest._value_Float.ToString();
			//		break;

			//	case TYPE.Vector2:
			//		strValue = calculatedRequest._value_Vec2.ToString();
			//		break;
			//}
			//Debug.LogWarning("Add ControlParam Request : [" + calculatedRequest._animPlayOrder + " (" + layer + ") ] : " + calculatedRequest._parentAnimClip._name + " / " + strValue);
			...
			_isUpdated = true;
			//_totalWeight += calculatedRequest._animWeight;
		}
		...
		public void CompleteOptLayerUpdate()
		{
			...
			//			_int_ClampedCalculatedLayer = Mathf.Clamp(Mathf.RoundToInt(_int_CalculatedLayer), _int_Min, _int_Max);
