	...
	public class apBone
	{
		...
		public void GUIUpdate(bool isRecursive = false, bool isBoneIKUsing = false)
		{
			...
			else
			{
				...
				//float boneWidth = BONE_V2_DEFAULT_WIDTH * s_boneRenderSettings._scaleRatio;
				float boneLength = _shapeLength * BONE_V2_REAL_LENGTH_RATIO;
				
				//if(!s_boneRenderSettings._isScaledByZoom)
				//{
				//	boneWidth *= 1.0f / s_boneRenderSettings._workspaceZoom;//Zoom에 반비례해야 항상 고정된 크기가 된다.
				//}
				//float boneRadius = boneWidth * 0.5f;
				...
				//선 굵기도 정하자
				float lineThick = BONE_V2_DEFAULT_LINE_THICKNESS * Mathf.Abs(_worldMatrix._scale.x) * Mathf.Abs(_worldMatrix._scale.y);
				if(!s_boneRenderSettings._isScaledByZoom)
				{
				...
				if (isBoneIKUsing)
				{
					...
					if (_isIKCalculated_Controlled || _isIKRendered_Controller)
					{
						...
						if (_worldMatrix_IK._scale.y != 0.0f)
						{
							backScaleYCorrection_IK = Mathf.Abs(_worldMatrix_IK._scale.x) / Mathf.Abs(_worldMatrix_IK._scale.y);//이렇게 해야 원형이 유지된다.
						}
